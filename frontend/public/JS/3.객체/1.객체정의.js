...
/*
	-JAVA / JS : 객체 지향 언어 [ OOP : obhect oriented programming ]
		1. 메모리/저장소
			1. 변수 : let 변수명 = 데이터							[데이터 1개]
			2. 상수 : const 상수명 = 데이터						[데이터 1개]
			3. 배열 : let 배열명 = [데이터1, 데이터2, 데이터3]		[데이터 여러개]
				-문제점 : 서로 다른 데이터 유형들을 한번에 저장시 식별이 힘듦
				* 3명의 아이디, 비밀번호, 이름을 저장
			4. 객체 : let 객체명 = { }
			
			
	- 객체[ Object ]
		1. 사용목적 : 서로 다른 데이터 유형(속성)들을 한번에 저장하기 위해
		2. 형태 : 
...
*/
 ...
 console.log('객체내 아이디 속성 : ' + 객체1.아이디) // 객체 내 아이디 속성의 값 호출
 console.log('객체내 비밀번호 속성 : ' +객체1.비밀번호)
 console.log('객체내 이름 속성 : ' +객체1.이름)
	//Vs
 console.log(배열1[0])
 
 // 3. 객체의 속성 추가 [선언시 추가 못했던 속성이 있을경우]
 ...
 // 6. 배열과 객체 관계 [배열 안에 객체 저장 가능 / 객체 안에 배열 저장 가능]
 	//1.배열 안에 여러개 객체 저장 [!! 동일한 객체의 유형]
 let 게시물1 = {제목:'제목입니다1.', 내용: '내용입니다1.'}
 let 게시물2 = {제목:'제목입니다2.', 내용: '내용입니다2.'}
