...
document.addEventListener('DOMContentLoaded', function(){
	...
	//2.
	for(let index in h1Array){		// 배열내 요소 인덱스를 마지막까지 하나씩 반복변수 대입
		h1Array[index].innerHTML = '안녕' + index
	} //for e
	//3.
	for(let object of h1Array){ //배열내 요소 모든 데이터를 마지막까지 하나씩 반복변수 대입
		object.innerHTML = '안녕'
	} // for e
	//4.
	h1Array.forEach((object)=>{ object.innerHTML='안녕~~~~'})
		...
		//배열명.forEach( (인수) => {실행문;} )
		//배열명.forEach( (요소값) => {실행문;} )
		h1Array.forEach( (object) => console.log(object))	//3번동일
