...
function gugudan(){ //함수 s
	//반복할때마다 초기화 되기 위해 선언당시에는 비우고 함수 시작시 단곤값 입력
	테이블행 = '<tr><th>단</th><th>곱</th><th>값</th></tr>'
	//<input> 마크업을 js변수로 가져오기 [DOM 객체]
	let temp1 = document.querySelector('.big')
	let temp2 = document.querySelector('.small')
...
//for문 i=1부터 입력된 곱까지, i * 입력된단을 sum에 대입한 후 table에 입력하는 것까지 반복
	for(i=1; i<=곱; i++){	//for s
		...
		sum = (단 * i);
		//테이블행에 추가
		테이블행 += '<tr><td>'+단+'</td><td>'+ i +'</td><td>'+sum+'</td></tr>'
	}	//for e
