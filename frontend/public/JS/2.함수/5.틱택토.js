...
//3. 결과 함수 [사람/ 컴퓨터 알두기 성공시 1.승리 2.패배 3. 무승부[빈자리가 없을때]]
function 결과(){
	//1.가로 승리자 판단
		//배열의 초기값이 모두 null이기 때문에 null 제외
	for(let i =0;i<=6;i+=3){ //for s
		if(버튼목록현황[i] != null && 버튼목록현황[i] == 버튼목록현황[i+1] && 버튼목록현황[i+1]==버튼목록현황[i+2]){
		결과출력(버튼목록현황[i])
		return 1;	//* 승리자 존재 하는 신호
		}
	} // for e
	//2.세로 승리자 판단
...
/*
	버튼목록현황
		...
		-승리판단 3가지
			1. 가로로 이겼을 때
			0 1 2 ,	3 4 5 , 6 7 8 모양이 모두 동일하면
			2. 세로로 이겼을 때
			0 3 6 ,	1 4 7 , 2 5 8 모양이 모두 동일하면
			3. 대각선으로 이겼을 때
			0 4 8, 2 4 6 모양이 모두 동일하면
		-무승부 : 알이 모두 존재 했을때 = 배열에 null이 0개 일때
