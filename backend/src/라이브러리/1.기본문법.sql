...
/*
	...
	- 종류
		...
		3. *관계형 데이터베이스		: 행과 열로 구성된 테이블[표] 간의 관계
	...
	예)
		DBA[표 만들어줘]------->DBMS[표 만들어줘--->기계어]----->DB[표 만들기 실행]
								메모리1 메모리2						메모리1
								메모리3 메모리4						메모리2
																메모리3
                                                                메모리4
	...
	- 테이블 [ 표 => 행[레코드] / 열[필드] ]
		*여러개의 자료들을 효율적으로 관리하기 위해서 관계형 데이터베이스[표]
            ...
            1열-colum	2열-colum
			아이디 		비밀번호
			qwe			1234			- 레코드1[1행-row]
            asd			4567			- 레코드2[2행-row]
            zxc			1597			- 레코드3[3행-row]
    
    
    
*/

-- 예제1 : 데이터베이스[저장소 - 여러개 테이블 저장할 수 있는 공간]
